...
public class DAOGenerator extends AbstractJavaClientGenerator {
    ...
    private Properties daoProperties;//optinal properies: superClass
    ...
    protected TopLevelClass getTopLevelClassShell() {
        ...
        if (daoProperties.get("superClass") != null) {
            daoTemplate.setCustomizedSuperClass(daoProperties.get("superClass")+"");
        }

//        FullyQualifiedJavaType annotationType = new FullyQualifiedJavaType("javax.lang.Repository");
        if (daoProperties.get("annotationClass") != null) {
            String annotationClass = daoProperties.getProperty("annotationClass");
            answer.addAnnotationClass(annotationClass);
            answer.addImportedType(annotationClass);
        }
